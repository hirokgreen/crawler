...
def get_headlines(soup_parser):
    try:
        headlines = soup_parser.find(
            "div", attrs={"class": "container"}).find(
...
def get_description_body(body):
    try:
        description = body.find(
            "div", attrs={"class": "details-text"}
        )
        [s.extract() for s in description('div')]
    except:
        return False
