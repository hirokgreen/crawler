...
def get_headlines(soup_parser):
    try:
        try:
            lead_headlines = soup_parser.find("div", attrs={"class": "mainPage"}).find("div", attrs={"class": "contentLeft"}).find_all("h4")
        except:
            lead_headlines = []
        headlines = soup_parser.find("div", attrs={"class": "mainPage"}).find("div", attrs={"class": "contentLeft"}).find_all("li")
        headlines = lead_headlines + headlines
...
def main():
    ...
    if not base_url:
        return
    req_url = os.path.join(base_url, 'news/111/0/')
